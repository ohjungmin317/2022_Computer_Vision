...
# 주사위 색상은 밝은 색 주사위 눈의 색상은 어두운색이고 배경은 밝은 배경

import numpy as np
import cv2 as cv
import math

img = cv.imread('light_dice1.PNG') # 원하는 개체 검출을 위한 이미지 받아오기
# * light_dice2 결과 값을 볼 때에는 해당 값을 주석 해제 해준다.
# img = cv.imread('light_dice2.jpg')
...
img_filter = cv.bilateralFilter(gray_img, -1, 47, 5) # 사진 잡음 처리
cv.imshow('img_filter', img_filter)

dect_edge = cv.Canny(img_filter, 30, 100) # canny 함수 이용하여 edge 검출
